...
func _get_minimum_size() -> Vector2:
	...
	return Vector2(64, max(35, size.x * ASPECT_RATIO))
...
func _draw():
	...
	var step_size: Vector2 = Vector2(_curve.get_domain_range(), _curve.get_value_range()) / grid_steps
	...
	for i in range(1, grid_steps.x):
		var x = _curve.min_domain + i * step_size.x
	...
	var range: float = _curve.get_value_range()
